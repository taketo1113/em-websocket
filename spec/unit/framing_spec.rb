...
describe EM::WebSocket::Framing03 do
  ...
  describe "basic examples" do
    it "connection close" do
      expect(@f).to receive(:message).with(:close, '', '')
      @f << 0b00000001
      @f << 0b00000000
    end
    
    it "ping" do
      expect(@f).to receive(:message).with(:ping, '', '')
      @f << 0b00000010
      @f << 0b00000000
    end
    
    it "pong" do
      expect(@f).to receive(:message).with(:pong, '', '')
      @f << 0b00000011
      @f << 0b00000000
    end
    
    it "text" do
      expect(@f).to receive(:message).with(:text, '', 'foo')
      @f << 0b00000100
      @f << 0b00000011
      @f << 'foo'
    end
    ...
    it "Text in two frames" do
      expect(@f).to receive(:message).with(:text, '', 'hello world')
      @f << 0b10000100
      @f << 0b00000110
      @f << "hello "
      @f << 0b00000000
    ...
    it "2 byte extended payload length text frame" do
      data = 'a' * 256
      expect(@f).to receive(:message).with(:text, '', data)
  ...
  describe "examples from the spec" do
    it "a single-frame text message" do
      expect(@f).to receive(:message).with(:text, '', 'Hello')
      @f << "\x04\x05Hello"
    end
    ...
    it "Ping request and response" do
      expect(@f).to receive(:message).with(:ping, '', 'Hello')
      @f << "\x02\x05Hello"
    end
    
    it "256 bytes binary message in a single frame" do
      data = "a"*256
      expect(@f).to receive(:message).with(:binary, '', data)
  ...
  describe "error cases" do
    it "should raise an exception on continuation frame without preceeding more frame" do
      expect {
        ...
        @f << 'f'
      }.to raise_error(EM::WebSocket::WebSocketError, 'Continuation frame not expected')
    end
  end
end
...
describe EM::WebSocket::Framing04 do
  ...
  describe "examples from the spec" do
    ...
    it "a pong response" do
      expect(@f).to receive(:message).with(:pong, '', 'Hello')
      @f << "\x83\x05Hello"
    end
  ...
  describe "other tests" do
    it "should raise a WSProtocolError if an invalid frame type is requested" do
      expect {
        ...
        @f << "\x83\x05Hello"
      }.to raise_error(EventMachine::WebSocket::WSProtocolError, "Unknown opcode 3")
    end
    ...
    it "should raise if non-fin frame is followed by a non-continuation data frame (continuation frame would be expected)" do
      expect {
        ...
        @f << 0b00000001 # Length 1
        @f << 'b'
      }.to raise_error(EM::WebSocket::WebSocketError, 'Continuation frame expected')
    end
    ...
    it "should raise on non-fin control frames (control frames must not be fragmented)" do
      ...
      }.to raise_error(EM::WebSocket::WebSocketError, 'Control frames must not be fragmented')
    end
  end
end
